...
public interface ProductImageRepository
  ...
  @Query(value = "SELECT * FROM PRODUCT_IMAGE pi2 LEFT JOIN" +
      " (SELECT p.pno,p.title, p.start_price, ROW_NUMBER() OVER (ORDER BY p.pno desc) AS RankNo " +
      " FROM PRODUCT p)pi3 " +
      " ON pi2.product_pno = pi3.pno" +
      " WHERE RankNo <= 6 AND pi2.inum IN (SELECT min(inum) FROM PRODUCT_IMAGE GROUP BY product_pno) ORDER BY RankNo ASC", nativeQuery = true)
  List<ProductImage> orderByPnoDesc();

  @Query(value = "SELECT pi2.INUM,pi2.IMG_NAME,pi2.UUID,pi2.PRODUCT_PNO,pi2.PATH  " +
      ...
      +
      " FROM PRODUCT_IMAGE GROUP BY product_pno) ORDER BY Ranknob ASC", nativeQuery = true)
  List<ProductImage> orderByBiddingDesc();

  @Query(value = "SELECT * FROM PRODUCT p LEFT JOIN (SELECT * FROM PRODUCT_IMAGE pi2)pi3 ON p.PNO = pi3.PRODUCT_PNO " +
      " where pi3.inum IN (SELECT min(inum) FROM PRODUCT_IMAGE GROUP BY product_pno) ORDER BY CREATED_DATE DESC", nativeQuery = true)
  List<ProductImage> getCategoryByCno();

  @Query(value = "SELECT * " +
      "FROM PRODUCT_IMAGE pi2 " +
      "WHERE pi2.PRODUCT_PNO IN (SELECT pno " +
      "FROM PRODUCT p " +
      "WHERE MEMBER_MID = 34) ", nativeQuery = true)
  List<ProductImage> getSaleProductImages(Long mid);
}

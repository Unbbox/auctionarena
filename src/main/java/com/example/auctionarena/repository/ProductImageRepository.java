...
public interface ProductImageRepository
  ...
  @Query(
    value = "SELECT * FROM PRODUCT p LEFT JOIN (SELECT * FROM PRODUCT_IMAGE pi2)pi3 ON p.PNO = pi3.PRODUCT_PNO " +
    " where pi3.inum IN (SELECT min(inum) FROM PRODUCT_IMAGE GROUP BY product_pno) ORDER BY CREATED_DATE DESC",
    nativeQuery = true
  )
  List<ProductImage> getCategoryByCno();

  @Query(
    value = "SELECT DISTINCT * from product p LEFT JOIN bidding b LEFT JOIN " +
    " (SELECT pi2.inum, pi2.img_name,pi2.product_pno FROM PRODUCT_IMAGE pi2)pi3 ON b.product_pno = pi3.product_pno " +
    " ON p.PNO = b.PRODUCT_PNO WHERE b.MEMBER_MID = ?1 AND pi3.inum IN (SELECT min(inum) FROM PRODUCT_IMAGE GROUP BY product_pno)",
    nativeQuery = true
  )
  List<ProductImage> findbiddingImageList(Long mid);
}

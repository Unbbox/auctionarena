...
public interface ProductImageRepository
  ...
  List<ProductImage> orderByPnoDesc(Long pno);
  ...
  @Query(
    ...
    " ON pi3.pno = b1.product_pno WHERE b1.product_pno IS NOT NULL AND RankNob < 7 AND pi2.inum IN (SELECT min(inum) FROM PRODUCT_IMAGE GROUP BY product_pno) ORDER BY Ranknob ASC",
    nativeQuery = true
  )
  List<ProductImage> orderByBiddingDesc(Long pno);
}
